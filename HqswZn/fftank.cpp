    ...
    ui(new Ui::FFTank)
{
    ...
    //ui->totalTimeValueLabel->setVisible(false);
    //ui->totalTimeLabel->setVisible(false);

    ui->startTimeLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->startTimeValueLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->endTimeLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->endTimeValueLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->weightTempture->setStyleSheet("QLabel {font: 11px;}");
    ui->weightTempture_2->setStyleSheet("QLabel {font: 11px;}");
    ui->wtValueLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->wtValueLabel_2->setStyleSheet("QLabel {font: 11px;}");
    ui->wtSybleLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->wtSybleLabel_2->setStyleSheet("QLabel {font: 11px;}");
    ui->wtValueLabel_4->setStyleSheet("QLabel {font: 11px;}");
    ui->wtValueLabel_5->setStyleSheet("QLabel {font: 11px;}");
    ui->runTimeLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->runTimeValueLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->SpaceTimeLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->spacTimeValueLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->totalTimeLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->totalTimeValueLabel->setStyleSheet("QLabel {font: 11px;}");
    ui->tankIndexLabel->setStyleSheet("QLabel {font: 18px;}");
...
void FFTank::switchState()
{
    ...
    if(isFerStart)
    {
        ui->totalTimeValueLabel->setText(formatLongDateString(latestFerRunTime + extraTotalRuntimeAddition));
...
void FFTank::parseFermentationData(QMap<float,QString> dataMap)
{
    ...
    DeviceNode deviceNode = Global::getFermenationNodeInfoByName("FER_WT_R_1");
    ...
    deviceNode = Global::getFermenationNodeInfoByName("FER_WT_R_2");
    address = deviceNode.Offset + (info.offset + deviceIndex - info.startIndex)
                            * Global::getLengthByDataType(deviceNode.DataType);
    temValue = dataMap.value(address).toFloat();
    if(temValue >= 0 && temValue <= 100)
    {
        ui->wtValueLabel_2->setText(QString::number(temValue, 'f', 2));
    }
    else
    {
        ui->wtValueLabel_2->setText("ERR");
    }
}
...
void FFTank::parseFerRunTimeData(QMap<float,QString> dataMap)
{
    ...
    address = deviceNode.Offset + (info.offset + deviceIndex - info.startIndex)
            * Global::getLengthByDataType(deviceNode.DataType);
    if(latestFerRunTime != dataMap[address].toUInt())
    {
