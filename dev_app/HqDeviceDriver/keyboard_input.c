...
//键盘输入 调试
//返回 -1 失败，返回0 成功
int  mHD_Debug_KeyboardInput_Init(void)
{
    pthread_t  a_thread;
    int arg;
    int res;
    //开一个线程  用来获取键盘输入 调试模式启用，非调试模式关闭
    res = pthread_create(&a_thread,NULL,mHD_keyboard_Scan,(void*)&arg);
    if(res !=0)  return -1;  //线程创建失败处
    return res;
}
