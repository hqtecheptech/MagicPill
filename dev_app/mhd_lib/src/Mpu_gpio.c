...
//卸载GPIO
//返回 0成功 -1 失败
int mHD_Uninstall_GPIO(int gpio)
{
    FILE * fp;
...
//返回 0成功 -1 失败
int mHD_SetGPIO_Direction(int gpio,char *direction)
{
    char path[100] = {0};
    FILE * fp;
...
//读取GPIO状态
//返回  -1 失败  其他GPIO 状态
int mHD_GetGPIO_Value(int gpio)
{
    char path[64];
    char value_str[3];
    int fd;
...
//设置GPIO 状态
//返回 -1 错误，
int mHD_Set_GPIO(int gpio,int value)
{
    int status;
    mHD_Request_GPIO(gpio);                         //申请GPIO
