...
DataReceiver::DataReceiver(QTcpSocket *tcpSocket, QObject *parent)
{
...
void DataReceiver::dataReceive()
{
    ...
    sData.append(_tcpSocket->readAll());
    ...
    StreamPack bDevice;
    memcpy(&bDevice, sData, sizeof(StreamPack));
    ...
    if(streamlen < StreamLength) return;
    ...
    {
        qDebug() << "r_AllCacheData";
        sendAllCachedData(bDevice);
    }
...
void DataReceiver::write(QString replyMsg)
{
...
void DataReceiver::sendAllCachedData(StreamPack pack)
{
    DataSender ds(_tcpSocket);
    QByteArray allPackData, SData, crcData;
    QDataStream out(&SData,QIODevice::WriteOnly);
...
quint32 DataReceiver::StreamLen_CRC32(QByteArray &data)
{
