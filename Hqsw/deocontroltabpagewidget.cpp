...
void DeoControlTabPageWidget::parseRunCtrData(QMap<float,QString> dataMap)
{
    ...
    if(isFault)
    {
        ui->sparyPumpLabel->setPixmap(filterPumpFaultBgImg);
    }
    else
    {
        if(isRan)
        {
            ui->sparyPumpLabel->setPixmap(filterPumpOpenedBgImg);
        }
        else
        {
            ui->sparyPumpLabel->setPixmap(filterPumpClosedBgImg);
        }
    }
